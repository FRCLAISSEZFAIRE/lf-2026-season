...
public class ShooterSubsystem extends SubsystemBase {
    ...
    // autoAimOffsetDeg moved to TunableNumber section (see turretOffsetDeg below)
    ...
    public ShooterSubsystem(Supplier<Pose2d> robotPoseSupplier) {
        ...
        // --- FLYWHEEL MOTOR (Kraken X60, CAN ID 13) ---
        flywheelMotor = new TalonFX(RobotMap.kShooterFollowerID);
    ...
    public void applyShooterState(ShooterState state) {
        setFlywheelRPM(state.rpm + flywheelOffsetTunable.get());
        setHoodAngle(state.hoodAngleDeg + hoodOffsetTunable.get());
    }
    ...
    public void periodic() {
        ...
        // Hub Offset Sync (bidirectional — hub offsets remain plain doubles)
        ...
        // Turret/Hood/Flywheel offsets are now TunableNumber — auto-synced via
        // Preferences
     ...
     * Calculates distance and angle to the Hub.
     * 
    ...
    public void updateAiming(Pose2d robotPose) {
        ...
        turretTargetDeg = -Math.toDegrees(targetTurretRad) + turretOffsetDeg.get();
        ...
        // --- CONSOLE LOG: Robot, Hub, Turret positions ---
        System.out.println("[Aiming] Robot: X=" + String.format("%.2f", robotPose.getX())
                + " Y=" + String.format("%.2f", robotPose.getY())
                + " Açı=" + String.format("%.1f", robotPose.getRotation().getDegrees()) + "°"
                + " | Hub: X=" + String.format("%.2f", hubLocation.getX())
                + " Y=" + String.format("%.2f", hubLocation.getY())
                + " | Mesafe=" + String.format("%.2f", distance) + "m"
                + " | Taret Hedef=" + String.format("%.1f", turretTargetDeg) + "°");
        ...
        Logger.recordOutput("Tuning/Shooter/Aiming/OffsetDeg", turretOffsetDeg.get());
    }
    ...
    // Live Offsets — TunableNumber (Dashboard'dan görülür ve düzenlenebilir,
    // kalıcı)
    private final TunableNumber turretOffsetDeg = new TunableNumber("Shooter/Offsets", "Turret", 0.0);
    private final TunableNumber hoodOffsetTunable = new TunableNumber("Shooter/Offsets", "Hood", 0.0);
    private final TunableNumber flywheelOffsetTunable = new TunableNumber("Shooter/Offsets", "Flywheel", 0.0);
    ...
    public void adjustHoodOffset(double deltaDeg) {
        hoodOffsetTunable.set(hoodOffsetTunable.get() + deltaDeg);
        System.out.println("[Offset] Hood: " + hoodOffsetTunable.get() + "°");
    }
    ...
    public void adjustFlywheelOffset(double deltaRPM) {
        flywheelOffsetTunable.set(flywheelOffsetTunable.get() + deltaRPM);
        System.out.println("[Offset] Flywheel: " + flywheelOffsetTunable.get() + " RPM");
    }

    public double getHoodOffset() {
        return hoodOffsetTunable.get();
    }

    public double getFlywheelOffset() {
        return flywheelOffsetTunable.get();
    }
    ...
    private void trackTarget() {
        ...
        // Turret offset is already applied in step 4 (turretOffsetDeg)
        hoodTargetDeg = state.hoodAngleDeg + hoodOffsetTunable.get();
        flywheelTargetRPM = state.rpm + flywheelOffsetTunable.get();
        ...
        Logger.recordOutput("Tuning/Shooter/Offsets/Turret", turretOffsetDeg.get());
        Logger.recordOutput("Tuning/Shooter/Offsets/Hood", hoodOffsetTunable.get());
        Logger.recordOutput("Tuning/Shooter/Offsets/Flywheel", flywheelOffsetTunable.get());
    }
    ...
    public void adjustAutoAimOffset(double deltaDeg) {
        turretOffsetDeg.set(turretOffsetDeg.get() + deltaDeg);
        System.out.println("[Offset] Turret: " + turretOffsetDeg.get() + "°");
    }
    ...
    private void logTelemetry() {
        ...
        Logger.recordOutput("Tuning/Shooter/AutoAim/OffsetDeg", turretOffsetDeg.get());
