...
![PhotonVision](https://img.shields.io/badge/PhotonVision-2026.2.1-purple)
...
│   ├── vision/                   # Limelight MegaTag2 (Pose Estimation)
...
| **Vision**  | Dual Limelight (Pose)     | -                         | **LimelightHelpers (MegaTag 2)**  |
| **Intake**  | Roller + Pivot + Camera   | Kraken + NEO              | **Phoenix6 + REVLib + PhotonLib** |
...
- **Limelight 3 (Right) & Limelight 3A (Left):** Çift kamera ile **MegaTag 2** kullanılarak robot pose estimation yapılır.
- **Intake Camera (Raspberry Pi + PhotonVision):** Game Piece algılama için **PhotonLib** kullanılır.
  - `IntakeSubsystem` içinde `PhotonCamera` ile renk pipeline üzerinden hedef takibi yapılır.
...
- **Custom MAXSwerve:** REV Robotics tabanlı özelleştirilmiş sürüş.
- **REVLib:** 2026.x (SparkMax / NEO)
- **Phoenix 6:** CTRE (Kraken / TalonFX)
- **PhotonLib:** 2026.2.1 (Intake Camera - Game Piece Detection)
- **LimelightHelpers:** Pose Estimation (MegaTag 2)
...
| Vision          | ✅ Tamamlandı (Dual LL3 + PhotonVision) |
